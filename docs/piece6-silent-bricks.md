...
| **TC/L1/L2 flush before 1st dispatch** | first kernel reads garbage from stale POST cache | concept sound; opcode to confirm | Emit `ACQUIRE_MEM` with full TC/L1/L2 flush before the first `DISPATCH_DIRECT`. (Confirm the ACQUIRE_MEM opcode; not present in this soc15d.h copy.) |
